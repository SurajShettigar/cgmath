...
namespace cgmath {
    ...
    /// Checks if two floating point values are equal. Because of floating point imprecision,
    ...
    inline bool approxEqual(FLOAT lhs, FLOAT rhs) {
        ...
        return std::abs(rhs - lhs)
               <= EPSILON * std::max(static_cast<FLOAT>(1.0), std::max(std::abs(lhs), std::abs(rhs)));
    }

    /// Checks if two floating point values are not equal. Because of floating point imprecision,
    /// != operator cannot be used directly.
