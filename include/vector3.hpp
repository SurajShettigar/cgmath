...
namespace cgmath {
    ...
    public:
        ...
        explicit Vector3(const std::array<FLOAT, 3> &value)
                : m_value{value} {};

        // Copy
        Vector3(const Vector3 &vec3) {
            if (this != &vec3)
                m_value = vec3.m_value;
        }
        ...
        friend Vector3 operator*(const Vector3 &lhs, FLOAT rhs) {
            return Vector3{lhs.m_value[0] * rhs,
                           lhs.m_value[1] * rhs,
                           lhs.m_value[2] * rhs};
        }
        ...
        friend Vector3 operator/(FLOAT lhs, const Vector3 &rhs) {
            return Vector3{lhs / rhs.m_value[0],
                           lhs / rhs.m_value[1],
                           lhs / rhs.m_value[2]};
        }
        ...
        /**
         * Find the dot product of two 3D vectors. Dot product of two normalized vectors gives the cosine of the angle
         * between them.
        ...
        /**
         * Find the angle between two 3D vectors. In other words, the first vector is rotated by that angle to produce the
         * second vector.
        ...
        /**
         * Find the cross product of two 3D vectors. By using two normalized, linearly independent
         * (not lying in the same plane) 3D vectors, we get another normalized 3D vector which is orthogonal / perpendicular
         * to these two vectors. Cross product is not commutative. Switching LHS and RHS gives a vector facing in the opposite
         * direction.
         * @param lhs First vector.
         * @param rhs Second vector.
         * @return A 3D vector. Vector is orthogonal to LHS and RHS vector if LHS and RHS are linearly independent.
         */
...
#endif
            return Vector3{lhs.m_value[1] * rhs.m_value[2] - lhs.m_value[2] * rhs.m_value[1],
        ...
        /**
         * Creates a coordinate frame / orthonormal basis around the given 3D vector. Useful for surface shading and forming
         * a basis for a camera.
         * @param normal Input 3D vector around which the coordinate frame is formed. For surface shading, the normal of the
         * surface (vector pointing out of the surface) is used. And for cameras, the camera's facing direction is used.
         * @param tangent Output 3D vector perpendicular to the normal vector. In case of surfaces, this vector forms a tangent
         * to the surface.
         * @param binormal Output 3D vector perpendicular to both normal and tangent vector.
         */
        static void getCoordinateFrame(const Vector3 &normal, Vector3 *tangent, Vector3 *binormal) {
            ...
            tangent->m_value[0] = 1.0 + z_sign * normal.m_value[0] * normal.m_value[0] * a;
